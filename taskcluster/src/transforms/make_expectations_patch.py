...
def add_all_expectations_deps(config, tasks):
    ...
    for task in tasks:
        ...
        soft_fetches.update({dep.label: { "artifact": "public/test_results/{}.toml".format(dep.attributes['apworld_name']), "dest": "/builds/worker/checkouts/vcs/meta/"} for dep in deps})
        yield task
